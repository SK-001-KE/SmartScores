...
function saveRecord() {
  ...
  const existing = records.find(r =>
    r.teacherName===teacherName && r.subject===subject && r.grade===grade &&
    r.stream===stream && r.term===term && r.examType===examType && r.year===year
  );
...
function getRubric(score){
  if(score>=75) return "Exceeding Expectations";
...
function renderSummary(){
  ...
  records.forEach(r=>{
    ...
    summary[key].total+=r.meanScore; summary[key].count+=1;
  });
...
function exportExcel(){
  ...
  a.download="SmartScores_Backup.json"; a.click();
}
...
function importExcel(event){
  ...
  reader.onload=e=>{
    ...
    }catch(err){ alert("⚠️ Failed to import: "+err.message);}
  };
...
function resetData(){
  if(confirm("⚠️ Reset all data? This cannot be undone.")){
    records=[]; localStorage.removeItem("smartScoresRecords");
    renderRecords(); renderSummary();
...
function downloadPDF(){
  ...
  const doc=new jsPDF();
  doc.setFontSize(18); doc.text("SmartScores Report",14,20);
  let startY=30;
  ...
  doc.setFontSize(14); doc.text("Average Score Summary",14,startY); startY+=6;
  ...
  startY=doc.lastAutoTable.finalY+10;
  doc.setFontSize(12); const insightsText=document.getElementById("insightBox").innerText;
  doc.text("Insights:",14,startY); startY+=6;
  doc.setFontSize(10); doc.text(doc.splitTextToSize(insightsText,180),14,startY);
