...
function clearForm(){
  teacher.value = '';
  subject.value = '';
  // grade, stream, term, exam, year remain for convenience
  score.value = '';
}
...
function renderRecords(){
  ...
  getFiltered().forEach((r, idx)=>{
    ...
    tr.innerHTML = `
      ...
      <td>${r.exam}</td>
      <td>${r.year}</td>
      <td style="color:${gradeInfo.color}; font-weight:600;">${r.score}</td>
      <td>
        <button onclick="editRecord(${idx})">✏️ Edit</button>
        <button onclick="deleteRecord(${idx})" style="background:#dc2626;">🗑️</button>
      </td>
    `;
...
function renderSummary(){
  ...
  Object.values(grouped).forEach(g=>{
    ...
    tbody.innerHTML += `<tr>
      ...
      <td style="color:${grading(overall).color}; font-weight:600;">${overall?overall.toFixed(1):''}</td>
    </tr>`;
...
// -------------------- RESET / EXPORT / IMPORT --------------------
function resetData(){
  if(confirm("⚠️ Reset all records?")){
    records=[];
